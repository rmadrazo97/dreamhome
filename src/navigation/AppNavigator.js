...
import {ExploreSvg,BookingsSvg,BookmarksSvg,ProfileSvg, SearchSvg, ChatSvg} from '../components/icons/TabbarSvgIcons';
	...
	const onStateChange = async (navState) => {
  	...
  	return (
	    <NavigationContainer ref={navigationRef} onStateChange={onStateChange}>
    		...
    		>
		        <Tab.Screen name="NearStack" component={NearStack} 
					options={({ route })=>{
						return {
							tabBarIcon: ({ color }) => {
							...
							tabBarLabel: ({ color }) => {
								return <TextRegular style={{fontSize: 13,color:color}}>{translate('nearme')}</TextRegular>
							},
						}
					}}
		        />
				{/* must un-comment this */}
				...
				<Tab.Screen name="ChatStack" component={ChatStack} 
					options={({ route })=>{
						return {
							...
							tabBarLabel: ({ color }) => {
								return <TextRegular style={{fontSize: 13,color:color,}}>Chat</TextRegular>
							},
						}
					}}
		        />
