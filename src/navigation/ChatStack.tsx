...
const ChatStack: React.FC<ChatStackProps> = ({ navigation, route }) => {
    const user = useSelector((state: any) => state.user);
    const { isLoggedIn } = user;
    const colors = getThemedColors(useColorScheme());

    useLayoutEffect(() => {
        const routeName = getFocusedRouteNameFromRoute(route) ?? (isLoggedIn ? 'Chat' : 'SignIn');
        navigation.setOptions({ tabBarVisible: routeName == 'Chat' });
    ...
    return (
        ...
        >
            {isLoggedIn ? (
                <>
                    <Stack.Screen name="Chat" options={{ title: translate('chat_screen') }}>
