...
class ClaimScreen extends React.Component{
    ...
    _onSubmit(){
        ...
        _self.props.submitClaim( {listing_id: _self.props.listing.ID, claim_message: details, user_id: _self.props.user.ID } )
        _self.setState({comSubmitted: true,validate: true})
    }
    errorButton(){
        return ( 
            <BtnLarge disabled={false} style={{marginTop:30}} onPress={ ()=>this.setState({validate: true}) }>{translate('claim','try_again')}</BtnLarge>
        )
    }
    successButton(){
        return ( 
            <BtnLarge disabled={false} style={{marginTop:30}} onPress={ ()=>{this.props.navigation.goBack()} }>{translate('claim','done')}</BtnLarge>
        )
    }
    render(){
        const _self = this;
        ...
        if( submitted ){
            popTitle = translate('claim', 'title_ok'); 
            if( submitError ) {
                popTitle = translate('claim', 'title_wrong');
            }
                ...
                {insets => <View style={[styles.container,{backgroundColor: apColors.appBg,paddingBottom: insets.bottom,paddingLeft: insets.left,paddingRight: insets.right}]}>
                    ...
                    <ScrollView style={{flex: 1}} contentContainerStyle={styles.contentContainer}>
                        <View style={styles.formWrap}>
                            ...
                            <TextRegular style={[styles.claimIntro,{color: apColors.pText,}]}>{translate('claim','intro')}</TextRegular>
                            
                            <TextRegular style={[styles.fieldLabel,{color: apColors.fieldLbl,}]}>{translate('claim','details')}</TextRegular>
                            <TextInput 
                            ...
                            <View style={styles.listingDetails}>
                                <TextHeavy style={[styles.listingTitle,{color: apColors.tText,}]}>{ translate( 'claim','listing', {title: _self.props.listing.title} ) }</TextHeavy>
                            </View>
                        </View>
                    </ScrollView>
